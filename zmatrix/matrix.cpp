...
//===========================================================================
int APIENTRY
_tWinMain(HINSTANCE hInstance, HINSTANCE hPrevInst, LPTSTR lpszArgs, int nWinMode)
{
	...
	if (LiteStepMode)
	{
		if (ghProgman != NULL)
		{
			ghWnd = ::CreateWindowEx(WS_EX_TRANSPARENT,
						szWinName, _TEXT("Matrix Code"),
						WS_CHILDWINDOW|WS_OVERLAPPED|WS_CLIPSIBLINGS|WS_CLIPCHILDREN,
						0, 0, gscreenWidth, gscreenHeight,
						ghProgman, NULL, hInstance, NULL);
		...
		else
		{
			ghWnd = ::CreateWindowEx(WS_EX_TRANSPARENT,
						szWinName, _TEXT("Matrix Code"),
						WS_CHILDWINDOW/*|WS_OVERLAPPED|WS_CLIPSIBLINGS*/|WS_CLIPCHILDREN,
						gscreenLeft, gscreenTop, gscreenWidth, gscreenHeight,
	...
	else
	{
		if (ghShellDLL != NULL)
		{
			ghWnd = ::CreateWindowEx(WS_EX_TRANSPARENT,
						...
						WS_CHILDWINDOW|WS_OVERLAPPED/*|WS_CLIPSIBLINGS*/|WS_CLIPCHILDREN,
						0, 0, gscreenWidth, gscreenHeight,
						ghShellDLL, NULL, hInstance, NULL);
			::EnableWindow(ghWnd, FALSE);
			::SetWindowPos(ghWnd, HWND_BOTTOM, 0,0,0,0, SWP_NOSIZE|SWP_NOMOVE);
		}
		else
...
{
	static int UWM_DESKTOPREDRAW = ::RegisterWindowMessage(UWM_DESKTOPREDRAW_ID);
	static int UWM_DESKTOPITEMCHANGED = ::RegisterWindowMessage(UWM_DESKTOPITEMCHANGED_ID);
	static int UWM_DESKTOPCHILDATTACHED = ::RegisterWindowMessage(UWM_DESKTOPCHILDATTACHED_ID);
	static POINT pt;
