...
enum FF
{
	FF_NoModifers         = 0x0000,          // none of the below (initialiser)
	FF_TextMode           = 0x0001,          // open file in text mode
	FF_IgnoreDisk         = 0x0002,          // don't search on disk, look straight in the .BIG file
	FF_DontUse0           = 0x0004,          // These bits are reserved for flag compatability with memory module
	FF_DontUse1           = 0x0008,
	FF_WriteMode          = 0x0010,          // open file for writing to
	FF_AppendMode         = 0x0020,          // open file for appending to
	FF_ReturnNULLOnFail   = 0x0040,          // open file, but if fail return NULL instead of doing Fatal Error
	FF_CDROM              = 0x0080,          // open from CD-ROM
	FF_IgnoreBIG          = 0x0100,          // don't look in .BIG file, only try to load from disk
	FF_IgnorePrepend      = 0x0200,          // don't environment root path to beginning of file names
	FF_UserSettingsPath   = 0x0400,          // use user configuration root as the base path
	FF_HomeworldDataPath  = 0x0800           // where the main data files are located
};
...
sdword fileLoadAlloc(char *fileName, void **address, enum FF flags);
sdword fileLoad(char *fileName, void *address, enum FF flags);
...
filehandle fileOpen(char *fileName, enum FF flags);
...
bool fileExists(char *fileName, enum FF flags);
sdword fileSizeGet(char *fileName, enum FF flags);
