...
static int bigFilenameRetrieve(bigTOC *toc, int fileNum, FILE *fp, char *filename)
{
    ...
    fseek(fp, (long)(toc->fileEntries + fileNum)->offset, SEEK_SET);
...
#else
	unsigned int numFiles = toc->numFiles;
	int flags    = toc->flags;
#endif
...
static int bigTOCRead(FILE *fp, bigTOC *toc)
{
    ...
    if (toc->numFiles)
    {
        toc->fileEntries = new bigTOCFileEntry[toc->numFiles];
...
static int bigHeaderVerify(FILE *fp)
{
    ...
    size_t headerLength = strlen(headerNeed);

    fgets(headerHave, (int)headerLength+1, fp);
...
static int bigSort(char *bigfilename)
{
    ...
    bigTOCSort(&toc);
    fseek(bigFP, (long)strlen(BF_FILE_HEADER BF_VERSION), SEEK_SET);
...
bool bigTOCFileExists(bigTOC *toc, char *filename, udword *fileNum)
{
    ...
    for (i = 0; ; i++) { filenamei[i] = tolower(filename[i]); }
    ...
    target.nameCRC1   = crc32Compute((ubyte *)filenamei,  (uint32_t)target.nameLength/2);
    target.nameCRC2   = crc32Compute((ubyte *)filenamei + target.nameLength/2, (uint32_t)target.nameLength/2);
    ...
    target.nameCRC1 = crc32Compute((ubyte *)filenamei, (uint32_t)halfFilenameLength);
    target.nameCRC2 = crc32Compute((ubyte *)filenamei + halfFilenameLength, (uint32_t)(target.nameLength - halfFilenameLength));

    return bigTOCFileExistsByCRC(toc, &target, fileNum) != 0;
}
...
//
void bigFilenameDecrypt(char *filename, size_t len)
{
    ubyte maskch = 213;
    while (len--)
    {
...
int bigPatch(char *oldfilename, char *newfilename, char *patchfilename, int consoleOutput)
{
    ...
    patchTOC.numFiles = 0;
    patchTOC.flags = 0;
    patchTOC.fileEntries = new bigTOCFileEntry[newTOC.numFiles];
    long curPatchOffset = (long)strlen(BF_FILE_HEADER BF_VERSION)
    ...
    for (i = 0; i < newTOC.numFiles; ++i)
    {
        ...
        if (bigTOCFileExistsByCRC(&oldTOC, newEntry, &fileNum))
        {
            ...
            else
            {
                ...
                fseek(oldFP, (long)(oldEntry->offset + oldEntry->nameLength+1), SEEK_SET);
                fseek(newFP, (long)(newEntry->offset + newEntry->nameLength+1), SEEK_SET);
                bytesCompared = 0;
    ...
    fseek(patchFP, (long)strlen(BF_FILE_HEADER BF_VERSION), SEEK_SET);
                    ...
                    case 2:
                        ...
                        if (fileEntry.compressionType)
                        {
                            ...
                            if ((float)compressedSize/(float)fileEntry.realLength > MINIMUM_COMPRESSION_RATIO)
                            {
                                ...
                                fseek(bigFP, curOffset + (long)strlen(tempshortfilename)+1, SEEK_SET);
                        ...
                        fseek(bigFP,   // overwrite placeholder from previous step
                              (long)strlen(BF_FILE_HEADER BF_VERSION) +
                                sizeof(toc.numFiles) +
                                sizeof(toc.flags) +
                        ...
                        curOffset += (long)fileEntry.nameLength + 1 + fileEntry.storedLength;
...
// reset back again
            ...
            case 1:
                ...
                curOffset = (long)strlen(BF_FILE_HEADER BF_VERSION)
                        + sizeof(toc.numFiles)
                        + sizeof(toc.flags)
...
int bigAddFile(char *bigFilename, char *filename, char *storedFilename, int optCompression, int optNewer, int consoleOutput)
{
    ...
    newtoc.fileEntries = NULL;//(bigTOCFileEntry *)0;
    ...
    // pretend name is lowercase for consistent CRCs
    for (i = 0; ; i++) { tempshortfilenamei[i] = tolower(storedFilename[i]); }
    ...
    else
    {
        ...
        newtoc.fileEntries = new bigTOCFileEntry[newtoc.numFiles];
        ...
        {
            // first entry
            fileEntry.offset = (long)strlen(BF_FILE_HEADER BF_VERSION) + sizeof(oldtoc.numFiles) + sizeof(oldtoc.flags) +
...
abort:
    ...
    {
        delete[] newtoc.fileEntries;
    }
    
    delete[] oldtoc.fileEntries;
    fclose(newfp);
    fclose(oldfp);
