...
static void fileNameReducePath (char* pathName)
{
	...
	assert(pathName != NULL); //dbgAssertOrIgnore(pathName != NULL);
	...
	for (i = 0; i < pathElemCount; i++)
	{
		...
		if (i == 0)
		{
			if (isPathAbsolute)
			{
				printf("fileNameReducePath(): Attempted to reach a "
		...
		assert(pathElemCount >= 2); //dbgAssertOrIgnore(pathElemCount >= 2);
		pathElemCount--;
		if (i < pathElemCount)
		{
			memmove(
...
static bool8 fileNameCorrectCase (char* fileName)
{
	...
	assert(fileName); //dbgAssertOrIgnore(fileName);

	/* Make a copy of the file name string to abuse as we wish. */
	assert(strlen(fileName) <= PATH_MAX); //dbgAssertOrIgnore(strlen(fileName) <= PATH_MAX);
	...
	if (isalpha(fileNameCopy[0]) && strcmp(fileNameCopy + 1, ":/") == 0)
	{
		...
		assert(strlen(fileNameCopy) <= strlen(fileName)); //dbgAssertOrIgnore(strlen(fileNameCopy) <= strlen(fileName));
	...
	assert(i < pathComponentCount); //dbgAssertOrIgnore(i < pathComponentCount);
	if (i == pathComponentCount)
	{
		...
		assert(pathComponentCount == 1); //dbgAssertOrIgnore(pathComponentCount == 1);
...
bool8 fileMakeDirectory (const char* directoryName)
{
	...
	assert(directoryName != NULL); //dbgAssertOrIgnore(directoryName != NULL);
	assert(strlen(directoryName) <= PATH_MAX); //dbgAssertOrIgnore(strlen(directoryName) <= PATH_MAX);
	...
	while (pChar)
	{
		*pChar = 0;
		...
		else
		{
			...
			if (_mkdir(directoryCopy) == -1)
#else
			if (_mkdir(directoryCopy, 0777) == -1)
#endif
				return FALSE;
		}
...
----------------------------------------------------------------------------*/
sdword fileLoadAlloc(char *_fileName, void **address, enum FF flags)
{
    ...
    assert(address != NULL); //dbgAssertOrIgnore(address != NULL);
    ...
    assert(nameLength > 1); //dbgAssertOrIgnore(nameLength > 1);  // end of the filename if filename too long
    ...
    assert(length > 0); //dbgAssertOrIgnore(length > 0);  // and verify it
    *address = memAllocAttempt(length, memoryName, (flags & (NonVolatile | Pyrophoric)) | MBF_String);//allocate the memory for this file
    if (*address == NULL)                                   //if couldn't allocate enough generate a fatal error
    {
		throw std::runtime_error("fileLoadAlloc: couldn't allocate"); //<< length << " bytes for " << fileName;
    }

    if ((inFile = fopen(fileName, "rb")) == NULL)           //open the file
    {
		throw std::runtime_error("fileLoadAlloc: coundn't open file"); //<< fileName;
    }
    ...
    {
		throw std::runtime_error("fileLoadAlloc: expected"); //<< length << " bytes, read " << lengthRead << " bytes";
    }
#endif
    ...
    printf("fileLoadAlloc: loaded %d bytes of '%s' to 0x%x from handle 0x%x", length, fileName, *address, inFile);
#endif
...
/*-----------------------------------------------------------------------------
    ...
    Note        : Generates a fatal error if file doesn't exist.  If you don't
        like this behavior, call fileExists() first.
----------------------------------------------------------------------------*/
sdword fileLoad(char *_fileName, void *address, enum FF flags)
{
    char  *fileName   = NULL;
    FILE  *inFile     = NULL;
    ...
    if ((inFile = fopen(fileName, "rb")) == NULL)            //open the file
    {
		throw std::runtime_error("fileLoadAlloc: couldn't open file"); //<< fileName;
    }
    lengthRead = fread(address, 1, length, inFile);         //read the file
    ...
    if (lengthRead != length)                               //verify correct amount read
    {
        throw std::runtime_error("fileLoadAlloc: expected"); // %d bytes, read %d bytes", length, lengthRead);
    }
#endif
    fclose(inFile);

#if FILE_VERBOSE_LEVEL >= 2
    printf("fileLoad: loaded %d bytes of '%s' to 0x%x", length, fileName, address);
#endif
...
sdword fileSave(char *_fileName, void *address, sdword length)
{
    ...
    {
		throw std::runtime_error("fileSave: unable to create destination directory for file"); //<< fileName
    }

    if ((outFile = fopen(fileName, "wb")) == NULL)           //open the file
    {
		throw std::runtime_error("fileSave: couldn't open file"); //<<fileName
    }
    ...
    {
        throw std::runtime_error("fileSave: expected"); //<< bytes, wrote %d bytes", length, lengthWrote);
    }
#endif
...
#if FILE_VERBOSE_LEVEL >= 2
    printf("fileSave: saved %d bytes of '%s' to 0x%x", length, fileName, address);
#endif
...
----------------------------------------------------------------------------*/
bool fileExists(char *_fileName, enum FF flags)
{
    char *fileName;
...
----------------------------------------------------------------------------*/
sdword fileSizeGet(char *_fileName, enum FF flags)
{
    FILE *file;
    sdword length;
    char *fileName;
    ...
    if (!IgnoreBigfiles && !bitTest(flags, FF_CDROM|FF_IgnoreBIG|FF_UserSettingsPath))
    {
        ...
        if (bigFindFile(_fileName, &whereFound, &fileIndex))
        {
            ...
            printf("%s: '%s' is %d bytes in length (bigfile)", __func__, _fileName, length);
#endif
            return length;
        }
    }
    ...
    {
        throw std::runtime_error("fileSizeGet: can't find file"); //<< fileName);
    }
    length = fseek(file, 0, SEEK_END);                      //get length of file
    assert(length == 0); //dbgAssertOrIgnore(length == 0);
    length = ftell(file);
    fclose(file);
#if FILE_VERBOSE_LEVEL >= 3
    printf("fileSizeGet: '%s' is %d bytes in length", fileName, length);
#endif
    return(length);
}
...
/*-----------------------------------------------------------------------------
 ...
 ----------------------------------------------------------------------------*/
filehandle fileOpen(char *_fileName, enum FF flags)
{
    FILE *file;
    char access[3];
    char *fileName;
    ...
    if (fh > MAX_FILES_OPEN)
    {
        ...
        throw std::runtime_error("fileOpen: too many files open - cannot open file"); //<<_fileName);
    }
    ...
    if (!localFileExists && !IgnoreBigfiles && !bitTest(flags, FF_CDROM|FF_IgnoreBIG|FF_UserSettingsPath))
    {
        ...
        if (bigFindFile(_fileName, &whereFound, &fileIndex))
        {
            ...
            assert(!bitTest(flags, FF_AppendMode)); //dbgAssertOrIgnore(!bitTest(flags, FF_AppendMode));
            assert(!bitTest(flags, FF_WriteMode)); //dbgAssertOrIgnore(!bitTest(flags, FF_WriteMode));
        ...
        if (usingBigfile)  // common stuff, whether it's in the main or update bigfile
        {
            if ((filesOpen[fh].bigTOC->fileEntries + fileIndex)->compressionType)
            {
                ...
                assert(expandedSize == filesOpen[fh].length); //dbgAssertOrIgnore(expandedSize == filesOpen[fh].length);
                assert(storedSize == (filesOpen[fh].bigTOC->fileEntries + fileIndex)->storedLength); //dbgAssertOrIgnore(storedSize == (filesOpen[fh].bigTOC->fileEntries + fileIndex)->storedLength);
            }
            else
            {
...
#if FILE_VERBOSE_LEVEL >= 2
            printf("fileOpen: '%s' (from bigfile) handle 0x%x", _fileName, fh);
#endif
    ...
    {
        throw std::runtime_error("fileOpen: unable to create destination directory for file"); //<<fileName);
    }
    ...
    if ((file = fopen(fileName, access)) == NULL)
    {
        ...
        throw std::runtime_error("fileOpen: cannot open file"); //<<fileName);
    }
    
#if FILE_VERBOSE_LEVEL >= 2
    printf("fileOpen: '%s' (from filesystem) handle 0x%x, FILE *0x%x", fileName, fh, file);
#endif
...
{
    assert(handle); //dbgAssertOrIgnore(handle);
    assert(filesOpen[handle].inUse); //dbgAssertOrIgnore(filesOpen[handle].inUse);
...
#if FILE_VERBOSE_LEVEL >= 2
    printf("fileClose: handle 0x%x", handle);
#endif
}
...
sdword fileSeek(filehandle handle, sdword offset, sdword whence)
{
    ...
    assert(!filesOpen[handle].textMode || (whence == FS_Start && !offset)); //dbgAssertOrIgnore(!filesOpen[handle].textMode || (whence == FS_Start && !offset));
    ...
    if (filesOpen[handle].usingBigfile)
    {
        ...
        assert(newLocation < filesOpen[handle].length); //dbgAssertOrIgnore(newLocation < filesOpen[handle].length);
    }
    else  // filesystem
    {
        newLocation = fseek(filesOpen[handle].fileP, offset, whence);
        assert(newLocation == 0); //dbgAssertOrIgnore(newLocation == 0);
        newLocation = ftell(filesOpen[handle].fileP);
    }

#if FILE_VERBOSE_LEVEL >= 3
    printf("fileSeek: handle 0x%x seeked %d bytes (mode %d) to %d", handle, offset, whence, newLocation);
#endif
...
sdword fileBlockRead(filehandle handle, void *dest, sdword nBytes)
{
    ...
    assert(handle); //dbgAssertOrIgnore(handle);
    assert(filesOpen[handle].inUse); //dbgAssertOrIgnore(filesOpen[handle].inUse);
    assert(!filesOpen[handle].textMode); //dbgAssertOrIgnore(!filesOpen[handle].textMode);

    if (filesOpen[handle].usingBigfile)
    {
        assert(filesOpen[handle].offsetVirtual + nBytes <= filesOpen[handle].length); //dbgAssertOrIgnore(filesOpen[handle].offsetVirtual + nBytes <= filesOpen[handle].length);
    ...
    {                                                       //make sure it was all read in
        throw std::runtime_error("fileBlockRead: expected"); //<< %d bytes, read %d", nBytes, lengthRead);
    }
#endif
#endif

#if FILE_VERBOSE_LEVEL >= 3
    printf("fileBlockRead: handle 0x%x read %d bytes to 0x%x", handle, lengthRead, dest);
#endif
...
sdword fileLineRead(filehandle handle, char *dest, sdword nChars)
{
    ...
    assert(nChars > 0); //dbgAssertOrIgnore(nChars > 0);  // validate the params
    assert(handle); //dbgAssertOrIgnore(handle);
    assert(dest != NULL); //dbgAssertOrIgnore(dest != NULL);
    assert(filesOpen[handle].inUse); //dbgAssertOrIgnore(filesOpen[handle].inUse);
    assert(filesOpen[handle].textMode); //dbgAssertOrIgnore(filesOpen[handle].textMode);
...
#if FILE_VERBOSE_LEVEL >= 3
    printf("fileLineRead: handle 0x%x read %d chars to 0x%x ('%s')", handle, length, dest, dest);
#endif
...
void filePathMaxBufferSet(char *buffer, char *path)
{
    ...
    assert(buffer); //dbgAssertOrIgnore(buffer != NULL);
    assert(path); //dbgAssertOrIgnore(path   != NULL);
    
    path_len = strlen(path);

    assert(path_len < PATH_MAX); //dbgAssertOrIgnore(path_len < PATH_MAX);
...
----------------------------------------------------------------------------*/
char *filePathPrepend(char *fileName, enum FF flags)
{
    assert(fileName); //dbgAssertOrIgnore(fileName != NULL);
...
void fileCDROMPathSet(char *path)
{
	...
	// TODO: check path to be drive root!
	UINT res = GetDriveType(path);
    if (res != DRIVE_CDROM)
    {
