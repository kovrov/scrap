...
void CRegistersPageWnd::OnPrepareAnimation()
{
	...
	for (int i = 0; i < 1000; i++ )
		pList->Add(new CListTextElementUI);    // We want 1000 items in list
}
...
{
	if (msg.sType == _T("itemexpand"))
		OnExpandItem(msg.pSender);
...
void CSystemsPageWnd::OnExpandItem(CControlUI* pControl)
{
	...
	pText->SetText(sText);
	pItem->Add(pText);
	pItem->Add((new CTextPanelUI())->ApplyAttributeList(_T("text=\"<b>Navn:</b> Anders And</b>\"")));
...
void CSearchPageWnd::Notify(TNotifyUI& msg)
{
	if (msg.sType == _T("click"))
	{
		...
		if (msg.pSender->GetName() == _T("cancel"))
			Close();
	}
	CStandardPageWnd::Notify(msg);
}
...
{
	if (msg.sType == _T("click") && msg.pSender->GetName() == _T("cancel"))
		Close();
