...
public:
	...
	void _CreatePage(CStdString sName)
	{
		...
		if (sName == "page_start")
			pWindow = new CStartPageWnd;
		if (sName == "page_configure")
			pWindow = new CConfigurePageWnd;
		if (sName == "page_reports")
			pWindow = new CReportsPageWnd;
		if (sName == "page_systems")
			pWindow = new CSystemsPageWnd;
		if (sName == "page_registers")
			pWindow = new CRegistersPageWnd;
		if (sName == "link_start")
			pWindow = new CStartPageWnd;
		if (sName == "link_configure")
			pWindow = new CConfigurePageWnd;
		if (sName == "link_reports")
			pWindow = new CReportsPageWnd;
		if (sName == "link_systems")
			pWindow = new CSystemsPageWnd;
		if (sName == "link_registers")
			pWindow = new CRegistersPageWnd;
		if (pWindow != NULL)
		{
		...
		if (sName == "page_search")
			pWindow = new CSearchPageWnd;
		if (sName == "page_edit")
			pWindow = new CEditPageWnd;
		if (pWindow != NULL)
		{
...
int APIENTRY WinMain(HINSTANCE hInstance, HINSTANCE /*hPrevInstance*/, LPSTR /*lpCmdLine*/, int nCmdShow)
{
	...
	if (::LoadLibrary("d3d9.dll") == NULL)
		::MessageBox(NULL, "Denne applikation vises bedst med DirectX 9 installeret!", "Test", MB_ICONINFORMATION);
